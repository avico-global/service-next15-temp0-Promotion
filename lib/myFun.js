...
export const getDomain = (host) => {
  ...
  // const defaultDomain = "sarasota-dryervent.sitebuilderz.com";
...
// Publish workflow: Delete old JSON files and fetch fresh data
export const publishProject = async ({ domain, tags = [] }) => {
  const isTemplateURL = checkDomain(domain);
  const isProjectStagingURL = domain?.endsWith("sitebuilderz.com");

  // Only apply publish workflow to live domains
  if (isTemplateURL || isProjectStagingURL) {
    console.warn("Publish workflow only applies to live domains");
    return {
      success: false,
      message: "Publish workflow only applies to live domains",
    };
  }

  if (typeof window !== "undefined") {
    console.warn("Publish workflow can only be executed on the server");
    return {
      success: false,
      message: "Publish workflow can only be executed on the server",
    };
  }

  const { deleteJson } = await import("./serverUtils");
  const results = [];

  try {
    // If no specific tags provided, get common tags
    const tagsToRefresh =
      tags.length > 0
        ? tags
        : [
    ...
    for (const tag of tagsToRefresh) {
      try {
        // Step 1: Delete old JSON file
        const baseURL = `${process.env.NEXT_PUBLIC_SITE_MANAGER}/api/public/project_data_by_domain/${domain}/data`;
        const fileName = baseURL.replace(
          `${process.env.NEXT_PUBLIC_SITE_MANAGER}/`,
          ""
        );
        ...
        // Step 2 & 3: Fetch fresh data and save as JSON
        const freshData = await callBackendApi({ domain, tag });
        ...
        if (freshData && !freshData.error) {
          ...
          results.push({
            tag,
            status: "error",
            message: freshData?.error || "Failed to fetch data",
          });
        }
      } catch (err) {
        console.error(`Error refreshing tag ${tag}:`, err);
    ...
    return {
      success: true,
      message: "Publish workflow completed",
      results,
    };
  } catch (err) {
    console.error("Error in publish workflow:", err);
    return {
      success: false,
