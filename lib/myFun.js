  ...
  if (
    ...
    try {
      ...
      if (cachedData && cachedData.data && cachedData.data.length > 0) {
        console.log(
          `📦 Cache HIT: Using cached bulk data for ${domain} (${cachedData.data.length} items)`
        );
        return cachedData;
      } else {
  ...
  if (
    ...
    try {
      ...
      if (cachedData && (cachedData.data || cachedData.error)) {
        ...
        console.log(
          `📦 Cache MISS: No valid cached ${tag} data found for ${domain}`
        );
      }
    } catch (err) {
      console.warn(
    ...
    console.log(
      `🔄 Force refresh requested for ${tag} on ${domain}, skipping cache`
    );
  }
