...
export const getDomain = (host) => {
  ...
  // const defaultDomain = "sanantonio-airduct.com";
...
// Get project info with JSON caching following the same pattern as callBackendApi
export const getProjectInfo = async ({ project_id, domain }) => {
  ...
  try {
    // Create file path for project_info JSON cache - use same folder as other API calls
    const filePath = `${domain}/api_public_project_data_by_domain_data/project_info.json`;
    ...
    if (
      ...
      try {
        ...
        console.warn(
          "Server utils not available for project_info, skipping JSON check:",
          err.message
        );
      }
    }
    ...
    // Save the response to JSON file (server-side only)
    if (
      typeof window === "undefined" &&
      !isTemplateURL &&
      !isProjectStagingURL
      ...
      try {
        const { saveJson } = await import("./serverUtils");
        await saveJson({ filePath, data: projectInfoData });
      } catch (err) {
        console.warn(
          "Server utils not available for project_info, skipping JSON save:",
          err.message
        );
      }
    }
